...
enum TokenType{
                ...
                ENDFILE, ERROR,
                ANDA,
                INT_TYPE, REAL_TYPE,BOOL_TYPE,BOOL_TRUE,BOOL_FALSE // added for extended types : Assignment 2
              };
...
const char* TokenTypeStr[]=
            {
                ...
                "EndFile", "Error",
                "Anda",
                "IntType", "RealType", "BoolType","BoolTrue","BoolFalse" // added for extended types : Assignment 2
...
TreeNode* NewExpr(CompilerInfo* pci, ParseInfo* ppi)
{
    ...
    if(ppi->next_token.type == BOOL_TRUE || ppi->next_token.type == BOOL_FALSE)
    {
            TreeNode* tree = new TreeNode;
            tree->node_kind = NUM_NODE;      // reuse NUM_NODE
            tree->expr_data_type = BOOLEAN;  // important
            tree->num = (ppi->next_token.type == BOOL_TRUE ? 1 : 0);
            tree->line_num = pci->in_file.cur_line_num;
...
Value Evaluate(TreeNode* node, SymbolTable* symbol_table, Value* variables)
{
    if(node->node_kind==NUM_NODE)
    {
        ...
        {
            v.type = BOOLEAN;
            v.bval = (node->num != 0);   // true=1, false=0
            return v;
        }
            // REAL literal
        if(node->expr_data_type == REAL)
...
int main()
{
    ...
    CompilerInfo compiler_info("test.txt", "output.txt", "debug.txt");
